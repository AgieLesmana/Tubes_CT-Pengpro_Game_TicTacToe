...
func main() {
	...
	fmt.Println("Input a decimal number (1.3 for row 1 column 3):")
	...
	for i = 1; i <= 9 && !finished; i++ {
		...
		for row < 1 || row > 3 || col < 1 || col > 3 || err != nil {
			fmt.Println("Invalid slot, try again:")
			_, err = fmt.Scan(&slot)
			row = int(slot)
			col = int(slot*10.0) % 10}
		...
		//PART 7: Checking All Winning Cases
		...
		//PART 8: Reprinting Board + Changing Players Logic + Winning Logic
		fmt.Println(e11, e12, e13)
		fmt.Println(e21, e22, e23)
		fmt.Println(e31, e32, e33)
		if i < 9 && !finished {
			if !empty {i = i - 1
			} else {if answer == "X" {answer = "O"} else {answer = "X"}}
			fmt.Print("Input slot for ", answer, ":\n")}}

	//PART 9: Output Game Results
	if finished {fmt.Print("Player ", answer, " wins!")} else {fmt.Print("It's a draw!")}}